...
> ⚠️ На Windows выполняйте команды этого руководства в **Git Bash**. Запуск целей Makefile из PowerShell и `cmd.exe` не поддерживается. Готовый Windows-бинарник Клиента можно запускать из обычного терминала Windows.

## 0. Проверить окружение

Проверьте необходимые инструменты и доступность Docker:
...
Все команды должны завершиться без ошибок. Если `docker info` не проходит, запустите Docker Desktop или Docker Engine.

Проект требует Go 1.26. При стандартном значении `GOTOOLCHAIN=auto` ранее установленная версия Go может автоматически загрузить требуемый toolchain при первой сборке.
...
Если используется `GOTOOLCHAIN=local`, установите Go 1.26 вручную.
...
> ⚠️ `RECORD_MASTER_KEY` необходимо хранить отдельно и защищённо вместе с резервной копией PostgreSQL. После замены или утраты ключа ранее сохраненные payload записей останутся в базе, но расшифровать их будет невозможно.
>
> При этом локальный зашифрованный кеш Клиента от `RECORD_MASTER_KEY` не зависит. Он защищен отдельным ключом, который получается из пароля пользователя через Argon2id. Поэтому сохранившийся локальный кеш можно открыть offline с правильным паролем, даже если серверный мастер-ключ утрачен.

> 👉 Команды OpenSSL для ручной генерации серверных секретов и локального набора TLS-сертификатов собраны в [🔑 отдельном руководстве](SECRETS_AND_TLS.md).
