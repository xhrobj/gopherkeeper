...
func TestResolveLocation(t *testing.T) {
	...
	first, err := ResolveLocation(baseDirectory, "alice")
	if err != nil {
		t.Fatalf("ResolveLocation() error = %v", err)
	}
	second, err := ResolveLocation(baseDirectory, "alice")
	if err != nil {
		t.Fatalf("ResolveLocation() repeated error = %v", err)
	}
	otherLogin, err := ResolveLocation(baseDirectory, "bob")
	if err != nil {
	...
	if first.AccountID == otherLogin.AccountID {
	...
	if strings.Contains(first.DatabaseFile, "alice") {
